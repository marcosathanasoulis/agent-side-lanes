...
CANONICAL = "https://github.com/marcosathanasoulis/agent-side-lanes"
ALLOWED_REMOTES = {
    CANONICAL,
    CANONICAL + ".git",
    "git@github.com:marcosathanasoulis/agent-side-lanes.git",
}
...
def available(runner: Callable[..., object] = subprocess.run) -> list[str]:
    ...
    tags = [value for value in git(["tag", "--list", "v*", "--sort=-v:refname"], runner).splitlines() if TAG.fullmatch(value)]
    return tags
